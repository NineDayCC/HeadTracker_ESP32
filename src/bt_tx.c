...
#define GATTS_NUM_HANDLE_HT 2   // 1 service handle + 1 characteristic handle
...
#define TEST_MANUFACTURER_DATA_LEN  0
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
                ...
                if (descr_value == 0x0001){
                    if (a_property & ESP_GATT_CHAR_PROP_BIT_NOTIFY){
                        ...
                        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, gl_profile_tab[PROFILE_A_APP_ID].char_handle,
                                                sizeof(bt_pwm_value), bt_pwm_value, false);
                    }
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ...
        conn_params.max_int = 10;   // Minimum Connection Interval (N * 1.25 ms)
        conn_params.min_int = 10;   // Maximum Connection Interval (N * 1.25 ms)
        conn_params.timeout = 100; // Supervision Timeout (N * 10 ms)
        ...
        led_bt_ctrl(1);     // turn led on
        break;
    }
    case ESP_GATTS_DISCONNECT_EVT:
        ESP_LOGI(GATTS_TAG, "ESP_GATTS_DISCONNECT_EVT, disconnect reason 0x%x", param->disconnect.reason);
        esp_ble_gap_start_advertising(&adv_params);
        led_bt_ctrl(0);     // turn led off
        break;
